...
textSurveyRouter.post('/', async (req: Request, res: Response) => {
   try {
    ...
    if (req.body.answers.length != 4) throw new Error('The request did not contain exactly 4 answers');

    const survey = textSurveyFactory.create(req.body.answers, req.body.userId);
    ...
    throw new Error('Failed to create survey');
      ...
      if (error instanceof InvalidRequestError) {
         logger.logToBoth(
            `TextSurveyRoutes POST (/) - Internal server error: ${error.message}`,
            LogLevel.INFO,
            error.stack
         );
